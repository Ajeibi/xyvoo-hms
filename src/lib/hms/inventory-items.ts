...
export const listItemTypes = (supabase: SupabaseClient, tenantId: string) =>
  listLookup(supabase, tenantId, "inventory_item_types") as Promise<InventoryItemTypeRow[]>;

export const createItemType = (supabase: SupabaseClient, params: { tenantId: string; name: string; code?: string; sortOrder?: number }) =>
  createLookup(supabase, "inventory_item_types", params) as Promise<{ row: InventoryItemTypeRow | null; error: string | null }>;
...
export const updateItemType = (
  ...
  id: string,
  patch: { name?: string; sortOrder?: number; isActive?: boolean },
) => updateLookup(supabase, "inventory_item_types", tenantId, id, patch) as Promise<{ row: InventoryItemTypeRow | null; error: string | null }>;
...
async function attachItemLookupNames(
  ...
  const [{ data: units }, { data: types }] = await Promise.all([
    ...
    typeIds.length
      ? supabase.schema("hotel").from("inventory_item_types").select("id,name").eq("tenant_id", tenantId).in("id", typeIds)
      : Promise.resolve({ data: [] }),
  ]);
  ...
  return rows.map((r) => ({
    ...
    item_type_name: typeNameById.get(r.item_type as string) ?? "Unknown type",
...
export async function listItems(
  supabase: SupabaseClient,
  tenantId: string,
  opts?: { categoryId?: string; activeOnly?: boolean; search?: string },
) {
  ...
  const { data } = await q;
  return attachItemLookupNames(supabase, tenantId, (data ?? []) as Record<string, unknown>[]);
}
