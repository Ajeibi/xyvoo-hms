...
import { createItemType, listItemTypes } from "@/lib/hms/inventory-items";
...
export async function GET(req: Request) {
  try {
    ...
    const itemTypes = await listItemTypes(auth.service, auth.tenant.id);
    return NextResponse.json({ itemTypes });
  } catch (e) {
    if (e instanceof z.ZodError) {
      return NextResponse.json({ error: e.issues[0]?.message }, { status: 400 });
    }
    console.error("[inventory/item-types GET]", e);
...
export async function POST(req: Request) {
  try {
    ...
    const { row, error } = await createItemType(auth.service, {
