...
export async function GET(req: Request) {
  try {
    ...
    const result = await getIncomeStatement(auth.service, auth.tenant.id, { dateFrom, dateTo });
    return NextResponse.json(result);
  } catch (e) {
    console.error("[income-statement GET]", e);
    return NextResponse.json({ error: "Failed to load income statement." }, { status: 500 });
  }
}
