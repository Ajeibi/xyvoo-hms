...
export async function GET(req: Request) {
  try {
    ...
    const rows = await getArAgingReport(auth.service, auth.tenant.id);
    return NextResponse.json({ rows });
  } catch (e) {
    console.error("[ar-aging GET]", e);
    return NextResponse.json({ error: "Failed to load AR aging report." }, { status: 500 });
  }
}
